...
- **見出し語と並び順は `claude/leap-word-list.md`（LEAP 2300語）に従う。** アプリの No.n ＝ LEAP の No.n
  （2026-09-22 時点で No.1〜650 まで一致済み。次に足すのは No.651 から）
...
- **語源から意味が離れている語には `why`（なぜこの意味？）を書く。** 語根の意味をたどっても単語の意味に届かない語
  （apologize, condition, danger など）に、別画面で開くコラムを付ける。書き方は仕様書 §4-3
  - 語源が意味に直結する語には**付けない**（全部に付けると「語源で分かる」実感が薄れる）
  - `hook`（答え合わせ画面に出る一言）と**同じ内容を書かない**。重複は `node check-all.js` が見つける
...
## 作業を助ける道具（`.claude/` の中。Git には入らない）
- サブエージェント `word-reviewer` … 単語を書いたあとの第三者チェック（10語ずつの3回目に使う）
- サブエージェント `ui-reviewer` … 画面を変えたあとの見た目・使いやすさ・データ互換チェック
- スキル `roots-publish-check` … Commit・Push する前の公開前チェック
- プレビュー `roots` … `python3 -m http.server 8000` をアプリ内のブラウザで開く
- 検査コマンド（validate / check-all / preview-picto）は確認なしで実行できるよう許可済み
